...
fixed_number_of_DGs = 7;         % Specify the number of DGs if fixed is enabled
...
function [c, ceq] = fixed_dg_constraint(DG_params, fixed_number_of_DGs, n)
    ...
    c = [];
    % Equality constraint: sum of DG_locations should equal fixed_number_of_DGs
    DG_locations = DG_params(1:n);
    ceq = sum(round(DG_locations)) - fixed_number_of_DGs;
end

% Add nonlinear constraint if using fixed number of DGs
...
options = optimoptions('ga', 'MaxGenerations', 100, 'PopulationSize', 50, 'Display', 'iter', 'FunctionTolerance', 1e-6);

% Run the Genetic Algorithm
[DG_opt, fval] = ga(objective_function, 2 * n, [], [], [], [], lb, ub, [], options);
