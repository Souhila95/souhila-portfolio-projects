...
function [optimized_w, optimized_theta, fitness_log] = optimize_pso(H_BS_RIS, H_RIS_Users, H_BS_Users, N_BS, N_RIS, P_BS_max, sigma2, numParticles, maxIterations, quantization_levels)
    particle_w = randn(N_BS, numParticles) + 1j * randn(N_BS, numParticles); 
    particle_theta = 2 * pi * rand(N_RIS, numParticles); 
    ...
    for iter = 1:maxIterations
        ...
        for i = 1:numParticles
            velocity_w(:, i) = 0.5 * velocity_w(:, i) + rand * (best_local_w(:, i) - particle_w(:, i)) + rand * (global_best_w - particle_w(:, i));
            velocity_theta(:, i) = 0.5 * velocity_theta(:, i) + rand * (best_local_theta(:, i) - particle_theta(:, i)) + rand * (global_best_theta - particle_theta(:, i));
        end
...
userCounts = 1:20;
capacities = zeros(size(userCounts)); 
