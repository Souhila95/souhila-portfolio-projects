...
disp('microgrid.dss file loaded and compiled successfully.');
...
kWhrated = 120; % Energy capacity (kWh)
...
DSSText.Command = 'New Monitor.VoltageMonitor element=line.ExchangetoMain terminal=1 mode=0';
% Re-run the simulation with updated parameters
DSSText.Command = 'Solve';
end

% Export voltage monitor data
DSSText.Command = 'Export monitor VoltageMonitor';

% Read the exported voltage data
voltage_data = readmatrix('microgrid_Mon_VoltageMonitor_1.csv');
time_steps = voltage_data(:, 1); % Time steps
voltage_magnitudes = voltage_data(:, 3); % Voltage for each phase

% Plot the voltage profile
figure;
plot(time_steps, voltage_magnitudes);
xlabel('Time (hours)');
ylabel('Voltage (pu)');
title('Voltage Profile at LoadBus Over 24 Hours');
