...
clear; clc; close all;
%%%%>Rician Factor versus achievable sum rate (bit/sec)*%%%%%
% Constants
...
function [optimized_w, optimized_theta, fitness_log] = optimize_ga(H_BS_RIS, H_RIS_Users, H_BS_Users, N_BS, N_RIS, P_BS_max, sigma2, maxGenerations, popSize, quantization_levels)
    ...
    for gen = 1:maxGenerations
        ...
        top_w = population_w(:, idx(1:ceil(popSize / 2))); % Select top individuals
        top_theta = population_theta(:, idx(1:ceil(popSize / 2)));
        ...
        for i = 1:(popSize / 2)
            ...
            child_w = (top_w(:, p1) + top_w(:, p2)) / 2; % Average beamforming
            child_theta = mean([top_theta(:, p1), top_theta(:, p2)], 2); % Average RIS phases
            new_population_w(:, end + 1) = child_w; % Add child
        ...
        % Ensure population size matches popSize
        if size(new_population_w, 2) < popSize
            % Add random individuals if necessary
            additional_w = randn(N_BS, popSize - size(new_population_w, 2)) + 1j * randn(N_BS, popSize - size(new_population_w, 2));
            additional_theta = 2 * pi * rand(N_RIS, popSize - size(new_population_theta, 2));
            new_population_w = [new_population_w, additional_w];
            new_population_theta = [new_population_theta, additional_theta];
        elseif size(new_population_w, 2) > popSize
            % Trim excess individuals
            new_population_w = new_population_w(:, 1:popSize);
            new_population_theta = new_population_theta(:, 1:popSize);
        end

        population_w = new_population_w;
        population_theta = new_population_theta;
    end
...
% Main Execution: Rician Factor vs Achievable Sum Rate
K_factors = 0:2:20; % Rician factors
sum_rates_ga = zeros(1, length(K_factors)); % GA sum rates
sum_rates_pso = zeros(1, length(K_factors)); % PSO sum rates
sum_rates_hybrid = zeros(1, length(K_factors)); % Hybrid GA-PSO sum rates

for idx = 1:length(K_factors)
    K_factor = K_factors(idx);
    ...
    sum_rates_ga(idx) = compute_sum_rate(ga_w, ga_theta, H_BS_RIS, H_RIS_Users, H_BS_Users, sigma2);

    % PSO Optimization
    [pso_w, pso_theta, ~] = optimize_pso(H_BS_RIS, H_RIS_Users, H_BS_Users, N_BS, N_RIS, P_BS_max, sigma2, 50, 20, quantization_levels);
    sum_rates_pso(idx) = compute_sum_rate(pso_w, pso_theta, H_BS_RIS, H_RIS_Users, H_BS_Users, sigma2);
    ...
    sum_rates_hybrid(idx) = compute_sum_rate(hybrid_w, hybrid_theta, H_BS_RIS, H_RIS_Users, H_BS_Users, sigma2);
end

% Plot Results
figure;
plot(K_factors, sum_rates_ga, '-o', 'LineWidth', 2, 'DisplayName', 'GA');
hold on;
plot(K_factors, sum_rates_pso, '-s', 'LineWidth', 2, 'DisplayName', 'PSO');
plot(K_factors, sum_rates_hybrid, '-d', 'LineWidth', 2, 'DisplayName', 'Hybrid GA-PSO');
grid on;
xlabel('Rician Factor (K)');
ylabel('Achievable Sum Rate (bps/Hz)');
title('Rician Factor vs Achievable Sum Rate for Various Methods');
legend show;
